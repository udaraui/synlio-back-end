...
export class UpdateCompanyWiseResourcePool1765571362162
  implements MigrationInterface
{
  name = 'UpdateCompanyWiseResourcePool1765571362162';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DELETE
                             FROM "typeorm_metadata"
                             WHERE "type" = 'VIEW'
                               AND "name" = 'company_wise_resourse_pool_view'`);
    ...
    await queryRunner.query(`
            ...
            GROUP BY
                ...
                d.id,
                owner.id;
        `); // Removed terminating semicolon is sometimes an issue
  }
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
            ...
            SELECT
                ...
                owner.email                        AS pool_owner_email,
                owner.mobile_number                AS pool_owner_mobile,
                -- c.id AS company_id, --- REMOVED in the rollback
                json_agg(
                  json_build_object(
                    'resourceId', r.id,
                    'firstName',  r.first_name,
            ...
            GROUP BY
                rp.id,
                c.id, // NEW: c.id must remain here because it's used in the JOIN condition
                d.id,
                owner.id;
        `);
  }
